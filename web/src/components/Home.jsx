...
const Home = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // tries to load data from all meal plans into one array
  ...
  return (
    <>
      <p>welcome {username} to croads wrapped MVP</p>
      <button onClick={() => console.log(data)}>raw data</button>
      ...
      {!loading && (
        <>
          ...
          <div>
            <h2>Flex Balance</h2>
            {
              data['On-Campus Meal Plan Flex Dollars Activity'][0][
                'New Balance'
              ]
            }
          </div>
          ...
          <div>
            <h2>Time Stats</h2>
            <p>Earliest: {formatTime(mealTimes['early'])}</p>
            <p>Latest: {formatTime(mealTimes['late'])}</p>
            <h3>Average Times</h3>
